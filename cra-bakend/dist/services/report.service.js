...
class ReportService {
     ...
     */
    async getActivitiesData(filters) {
     ...
     */
    async getConventionsData(filters) {
        const period = quarter_service_1.quarterService.getReportPeriod(filters);
        const whereClause = {
            OR: [
                {
     ...
     */
    async getKnowledgeTransfersData(filters) {
        const period = quarter_service_1.quarterService.getReportPeriod(filters);
        const whereClause = {
            date: {
                gte: period.startDate,
     ...
     */
    async getQuarterlyStatistics(filters) {
        const period = quarter_service_1.quarterService.getReportPeriod(filters);
        const [totalActivities, newActivities, reconductedActivities, closedActivities, totalConventions, totalTransfers, totalBudget] = await Promise.all([
