...
exports.publicationService = exports.PublicationService = void 0;
...
class PublicationService {
    ...
    async getPublications(query, userId, userRole) {
    ...
    async updatePublication(id, data, userId) {
    ...
    async deletePublication(id, userId, userRole) {
        const publication = await prisma.publication.findUnique({
            where: { id },
            include: {
                authors: true
        ...
        const isAuthor = publication.authors.some(author => author.userId === userId);
        const isAdmin = userRole === client_1.UserRole.ADMINISTRATEUR;
        if (!isAuthor && !isAdmin) {
    ...
    async getResearcherPublications(researcherId, year) {
        const where = {
            authors: {
                some: {
                    userId: researcherId
    ...
    async getPublicationStats(userId) {
        ...
        const [total, byType, byYear, byQuartile, international] = await Promise.all([
            prisma.publication.count({ where }),
    ...
    async attachDocument(publicationId, file, userId) {
        ...
        if (publication.documentId) {
            ...
            if (oldDoc) {
                ...
                if (fs_1.default.existsSync(oldDoc.filepath)) {
                    fs_1.default.unlinkSync(oldDoc.filepath);
                }
    ...
    async downloadDocument(publicationId) {
        const publication = await prisma.publication.findUnique({
            where: { id: publicationId },
        ...
        if (!fs_1.default.existsSync(publication.document.filepath)) {
...
exports.publicationService = new PublicationService();
