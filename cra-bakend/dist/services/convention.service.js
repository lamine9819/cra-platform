...
class ConventionService {
    async createConvention(data, userId, userRole) {
        ...
        if (data.contractNumber) {
            ...
            if (existing) {
                throw new errors_1.ValidationError('Ce numéro de contrat existe déjà');
            }
        }
    ...
    async listConventions(userId, userRole, query) {
        ...
        if (query.responsibleId)
            where.responsibleUserId = query.responsibleId;
        if (query.startDate) {
    ...
    async getConventionById(conventionId, userId, userRole) {
        ...
        if (userRole !== 'ADMINISTRATEUR' && convention.responsibleUserId !== userId) {
            throw new errors_1.AuthError('Accès refusé à cette convention');
        }
        return this.formatConventionResponse(convention);
    }
    async updateConvention(conventionId, updateData, userId, userRole) {
        const convention = await prisma.convention.findUnique({
            where: { id: conventionId }
        });
        if (!convention) {
        ...
        if (updateData.contractNumber !== undefined)
            dataToUpdate.contractNumber = updateData.contractNumber || null;
        if (updateData.totalBudget !== undefined)
            dataToUpdate.totalBudget = updateData.totalBudget;
        if (updateData.documentPath !== undefined)
            dataToUpdate.documentPath = updateData.documentPath || null;
        if (updateData.mainPartner !== undefined)
            dataToUpdate.mainPartner = updateData.mainPartner;
        if (updateData.otherPartners !== undefined)
            dataToUpdate.otherPartners = updateData.otherPartners;
        if (updateData.responsibleUserId !== undefined)
            dataToUpdate.responsibleUserId = updateData.responsibleUserId;
    ...
    async deleteConvention(conventionId, userId, userRole) {
        const convention = await prisma.convention.findUnique({
            where: { id: conventionId },
            include: {
                _count: {
        ...
        if (userRole !== 'ADMINISTRATEUR') {
            throw new errors_1.AuthError('Seul un administrateur peut supprimer une convention');
        }
        ...
        if (convention._count.activities > 0 || convention._count.projects > 0 || totalFundings > 0) {
            throw new errors_1.ValidationError('Impossible de supprimer une convention liée à des activités, projets ou financements');
        }
        await prisma.convention.delete({
            where: { id: conventionId }
        });
    }
    formatConventionResponse(convention) {
        return {
            id: convention.id,
