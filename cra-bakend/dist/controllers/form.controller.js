...
class FormController {
    constructor() {
        ...
        // =============================================
        this.createForm = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const validatedData = formValidation_1.createFormSchema.parse(req.body);
        ...
        this.listForms = async (req, res, next) => {
            try {
                const authenticatedReq = req;
        ...
        this.getFormById = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const userId = authenticatedReq.user.userId;
        ...
        this.updateForm = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const validatedData = formValidation_1.updateFormSchema.parse(req.body);
                const userId = authenticatedReq.user.userId;
                const userRole = authenticatedReq.user.role;
        ...
        this.deleteForm = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const userId = authenticatedReq.user.userId;
        ...
        // =============================================
        this.shareFormWithUser = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const validatedData = formValidation_1.shareFormSchema.parse(req.body);
                const userId = authenticatedReq.user.userId;
                const userRole = authenticatedReq.user.role;
                const share = await formService.shareFormWithUser(id, validatedData.targetUserId, {
                    canCollect: validatedData.canCollect,
                    canExport: validatedData.canExport
                }, userId, userRole, validatedData.maxSubmissions, validatedData.expiresAt ? new Date(validatedData.expiresAt) : undefined);
                res.status(201).json({
                    success: true,
        ...
        this.createPublicShareLink = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const validatedData = formValidation_1.publicShareSchema.parse(req.body);
                const userId = authenticatedReq.user.userId;
                const userRole = authenticatedReq.user.role;
        ...
        this.getFormByPublicLink = async (req, res, next) => {
            try {
        ...
        this.getFormShares = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const userId = authenticatedReq.user.userId;
        ...
        this.removeFormShare = async (req, res, next) => {
            try {
                const authenticatedReq = req;
        ...
        // =============================================
        this.submitFormResponse = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const validatedData = formValidation_1.submitFormResponseSchema.parse(req.body);
                ...
                const response = await formService.submitFormResponse(id, {
                    ...validatedData,
                    photos: validatedData.photos
                        ...
                        : undefined
                }, respondentId, respondentRole, collectorInfo);
                res.status(201).json({
                    success: true,
        ...
        this.submitPublicFormResponse = async (req, res, next) => {
            try {
                const { shareToken } = req.params;
                const validatedData = formValidation_1.submitFormResponseSchema.parse(req.body);
                const formInfo = await formService.getFormByPublicToken(shareToken);
                ...
                const response = await formService.submitFormResponse(formId, {
                    ...validatedData,
                    photos: validatedData.photos
                        ? validatedData.photos.map(photo => ({
                            ...photo,
        ...
        this.getFormResponses = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const userId = authenticatedReq.user.userId;
        ...
        // GESTION DES PHOTOS
        // =============================================
        this.uploadFieldPhoto = async (req, res, next) => {
            try {
                if (!req.file) {
        ...
        this.uploadMultiplePhotos = async (req, res, next) => {
            try {
        ...
        this.getResponsePhotos = async (req, res, next) => {
            try {
                const authenticatedReq = req;
        ...
        // =============================================
        this.storeOfflineData = async (req, res, next) => {
            try {
                const validatedData = formValidation_1.offlineDataSchema.parse(req.body);
                await formService.storeOfflineData(validatedData.formId, validatedData.deviceId, validatedData.data);
                res.status(201).json({
                    success: true,
        ...
        this.syncOfflineData = async (req, res, next) => {
            try {
                const validatedData = formValidation_1.syncOfflineSchema.parse(req.body);
        ...
        this.getOfflineSyncStatus = async (req, res, next) => {
            try {
        ...
        // EXPORT DES DONNÉES
        // =============================================
        this.exportResponses = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const validatedQuery = formValidation_1.exportQuerySchema.parse(req.query);
                const userId = authenticatedReq.user.userId;
                const userRole = authenticatedReq.user.role;
        ...
        this.addComment = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const validatedData = formValidation_1.addCommentSchema.parse(req.body);
        ...
        this.getFormComments = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const { id } = req.params;
                const userId = authenticatedReq.user.userId;
        ...
        // =============================================
        this.getCollectorDashboard = async (req, res, next) => {
            try {
                const authenticatedReq = req;
                const userId = authenticatedReq.user.userId;
        ...
        this.previewForm = async (req, res, next) => {
            try {
                const { schema } = req.body;
                const validatedSchema = formValidation_1.createFormSchema.pick({ schema: true }).parse({ schema });
    ...
    // =============================================
    async checkIfFormIsShared(formId, userId) {
    ...
    checkResponseAccess(response, userId, userRole) {
        if (userRole === 'ADMINISTRATEUR')
            return true;
        if (response.form.creatorId === userId)
            return true;
        if (response.respondentId === userId)
            return true;
        if (response.form.activity?.project?.participants?.some((p) => p.userId === userId && p.isActive))
            return true;
        return false;
    }
    formatResponsesForExport(responses, form, options) {
        ...
        return responses.map((response) => {
            ...
            if (response.respondent) {
                ...
                const info = response.collectorInfo;
            ...
            if (options.includePhotos && response.photos) {
                ...
                exportRow['URLs des photos'] = response.photos.map((p) => `/uploads/photos/${p.filename}`).join(', ');
            }
            return exportRow;
        });
    }
    generatePreviewData(schema) {
        const previewData = {};
        if (schema.fields) {
