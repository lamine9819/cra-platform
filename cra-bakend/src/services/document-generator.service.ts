...
export class DocumentGeneratorService {
      ...
      ...activities.map(
        activity =>
          new TableRow({
            children: [
              new TableCell({
                children: [new Paragraph(activity.intitule)]
              }),
              new TableCell({
                children: activity.responsables.map(
                  resp => new Paragraph(resp)
                )
              }),
              new TableCell({
                children: [
    ...
    return new Promise((resolve, reject) => {
      ...
      conventions.forEach((conv, index) => {
        ...
        doc
          ...
          .stroke()
          .text(
            reportService.formatDate(conv.dateDebut),
            margins.col2 + 5,
            rowY + 10,
        ...
        doc
          ...
          .stroke()
          .text(
            reportService.formatDate(conv.dateFin),
            margins.col3 + 5,
            rowY + 10,
        ...
        doc
          ...
          .stroke()
          .text(
            reportService.formatCurrency(conv.financementGlobal),
            margins.col4 + 5,
            rowY + 10,
        ...
        doc
          ...
          .stroke()
          .text(
            reportService.formatCurrency(conv.financementMobilise),
            margins.col5 + 5,
            rowY + 10,
            { width: 100, align: 'right' }
          );
      ...
      ...conventions.map(
        conv =>
          new TableRow({
            children: [
              new TableCell({
                children: [new Paragraph(conv.intitule)]
              }),
              new TableCell({
                children: [
                  new Paragraph({
                    text: reportService.formatDate(conv.dateDebut),
                    alignment: AlignmentType.CENTER
                  })
                ]
              }),
              new TableCell({
                children: [
                  new Paragraph({
                    text: reportService.formatDate(conv.dateFin),
                    alignment: AlignmentType.CENTER
                  })
                ]
              }),
              new TableCell({
                children: [
                  new Paragraph({
                    text: reportService.formatCurrency(conv.financementGlobal),
              ...
              new TableCell({
                children: [
                  new Paragraph({
                    text: reportService.formatCurrency(conv.financementMobilise),
                    alignment: AlignmentType.RIGHT
                  })
                ]
              }),
              new TableCell({
                children: [new Paragraph(conv.bailleurs.join(', '))]
              })
            ]
          })
      )
   ...
   */
  async generateKnowledgeTransfersPDF(
    transfers: KnowledgeTransferReport[],
    filters: GenerateReportInput
  ): Promise<string> {
    const period = quarterService.getReportPeriod(filters);
    ...
    return new Promise((resolve, reject) => {
      ...
      transfers.forEach((transfer, index) => {
        ...
        doc
          ...
          .stroke()
          .text(
            reportService.formatDate(transfer.dateDisponibilite),
            margins.col2 + 5,
            rowY + 10,
   ...
   */
  async generateKnowledgeTransfersWORD(
    transfers: KnowledgeTransferReport[],
    filters: GenerateReportInput
  ): Promise<string> {
    const period = quarterService.getReportPeriod(filters);
      ...
      ...transfers.map(
        transfer =>
          new TableRow({
            children: [
              new TableCell({
                children: [new Paragraph(transfer.intitule)]
              }),
              new TableCell({
                children: [
                  new Paragraph({
                    text: reportService.formatDate(transfer.dateDisponibilite),
                    alignment: AlignmentType.CENTER
                  })
                ]
              }),
              new TableCell({
                children: [new Paragraph(transfer.description)]
              }),
              new TableCell({
                children: [new Paragraph(transfer.impactPotentiel)]
              }),
              new TableCell({
                children: [new Paragraph(transfer.cibles.join(', '))]
              })
            ]
          })
      )
   ...
   */
  private generateCoverPage(doc: PDFKit.PDFDocument, title: string, year: number): void {
    doc
      .fontSize(28)
      .font('Helvetica-Bold')
  ...
  private generateAnnualSummaryPage(
    ...
    doc.text(
      `Budget mobilisé: ${reportService.formatCurrency(statistics.annual.budget.totalMobilized)}`,
      300,
      startY + 40
    );
    ...
    [Quarter.Q1, Quarter.Q2, Quarter.Q3, Quarter.Q4].forEach((quarter) => {
    ...
    const maxActivities = Math.max(
      ...Object.values(statistics.byQuarter).map((q: any) => q.activities.total)
    );
  ...
  private generateQuarterActivitiesPage(
    ...
    const quarterLabel = quarterService.formatQuarterLabel(year, quarter);
    ...
    activities.forEach((activity, index) => {
      ...
      doc.text(
        activity.responsables.length > 2
          ? `${responsablesText}... (+${activity.responsables.length - 2})`
          : responsablesText,
        col2X + 5,
        rowY + 10,
        { width: 160, height: rowHeight - 20 }
      );
  ...
  private generateConclusionPage(
    ...
    const highlights = [
      ...
      `${reportService.formatCurrency(statistics.annual.budget.totalMobilized)} de budget mobilisé`
    ];
    ...
    const q1 = statistics.byQuarter[Quarter.Q1].activities.total;
    const q4 = statistics.byQuarter[Quarter.Q4].activities.total;
    const trend = q4 > q1 ? 'hausse' : q4 < q1 ? 'baisse' : 'stabilité';
    const trendPercent = q1 > 0 ? (((q4 - q1) / q1) * 100).toFixed(1) : 0;

    doc.text(
      `Une tendance à la ${trend} a été observée entre le premier et le dernier trimestre (${trendPercent}%).`,
      70,
      doc.y
    );
    doc.moveDown(1);
    ...
    const mostActiveQuarter = Object.entries(statistics.byQuarter).reduce((max, [q, stats]: any) =>
      stats.activities.total > max.stats.activities.total ? { quarter: q, stats } : max,
      { quarter: Quarter.Q1, stats: statistics.byQuarter[Quarter.Q1] }
    );

    doc.text(
      `Le T${mostActiveQuarter.quarter} a été le trimestre le plus actif avec ${mostActiveQuarter.stats.activities.total} activités.`,
      70,
      doc.y
    );
   ...
   */
  async generateAnnualActivitiesPDF(
    activitiesByQuarter: Map<Quarter, ActivityReport[]>,
    year: number,
    statistics: any
  ): Promise<string> {
   ...
   */
  async generateAnnualActivitiesWORD(
    activitiesByQuarter: Map<Quarter, ActivityReport[]>,
    year: number,
    statistics: any
  ): Promise<string> {
    ...
    sections.push({
      children: [
        ...
        new Paragraph({
          text: `Budget mobilisé: ${reportService.formatCurrency(statistics.annual.budget.totalMobilized)}`,
          spacing: { after: 400 }
        }),
  ...
  if (activities.length > 0) {
    sections.push({
      children: [
        new Paragraph({
          text: quarterService.formatQuarterLabel(year, quarter),
          heading: HeadingLevel.HEADING_1,
          spacing: { after: 400 }
        }),
   ...
   */
  private createQuarterlySummaryTable(statistics: any): Table {
  const rows: TableRow[] = [
    new TableRow({
      tableHeader: true,
      children: [
        new TableCell({
          children: [
            new Paragraph({
              children: [new TextRun({ text: 'Trimestre', bold: true })]
            })
          ],
          shading: { fill: 'CCCCCC' }
        }),
        new TableCell({
          children: [
            new Paragraph({
              children: [new TextRun({ text: 'Activités', bold: true })]
            })
          ],
          shading: { fill: 'CCCCCC' }
        }),
        new TableCell({
          children: [
            new Paragraph({
              children: [new TextRun({ text: 'Nouvelles', bold: true })]
            })
          ],
          shading: { fill: 'CCCCCC' }
        }),
        new TableCell({
          children: [
            new Paragraph({
              children: [new TextRun({ text: 'Reconduites', bold: true })]
            })
          ],
          shading: { fill: 'CCCCCC' }
        }),
        new TableCell({
          children: [
            new Paragraph({
              children: [new TextRun({ text: 'Clôturées', bold: true })]
            })
          ],
          shading: { fill: 'CCCCCC' }
        })
    ...
    [Quarter.Q1, Quarter.Q2, Quarter.Q3, Quarter.Q4].forEach((quarter) => {
      ...
      rows.push(
        new TableRow({
          children: [
            new TableCell({
              children: [new Paragraph(`T${quarter}`)]
            }),
            new TableCell({
              children: [
                new Paragraph({
   ...
   */
 private createActivitiesTable(activities: ActivityReport[]): Table {
  const rows: TableRow[] = [
    new TableRow({
      tableHeader: true,
      children: [
        new TableCell({
          children: [
            new Paragraph({
              children: [
                new TextRun({ text: 'Intitulé activité', bold: true })
              ]
            })
          ],
          width: { size: 40, type: WidthType.PERCENTAGE },
        ...
        new TableCell({
          children: [
            new Paragraph({
              children: [
                new TextRun({ text: "Responsables d'activités", bold: true })
              ]
            })
          ],
          width: { size: 35, type: WidthType.PERCENTAGE },
        ...
        new TableCell({
          children: [
            new Paragraph({
              children: [
                new TextRun({ text: 'Situation', bold: true })
              ]
            })
          ],
          width: { size: 25, type: WidthType.PERCENTAGE },
      ...
      rows.push(
        new TableRow({
          children: [
            ...
            new TableCell({
              children: activity.responsables.map((resp) => new Paragraph(resp))
            }),
            new TableCell({
              children: [
                new Paragraph({
   ...
   */
  private createHighlightsParagraphs(statistics: any): Paragraph[] {
    const highlights = [
      `${statistics.annual.activities.total} activités menées au total`,
      `${statistics.annual.activities.new} nouvelles activités lancées`,
      `${statistics.annual.activities.reconducted} activités reconduites`,
   ...
   */
  private createTrendsParagraphs(statistics: any): Paragraph[] {
    const q1 = statistics.byQuarter[Quarter.Q1].activities.total;
    const q4 = statistics.byQuarter[Quarter.Q4].activities.total;
    const trend = q4 > q1 ? 'hausse' : q4 < q1 ? 'baisse' : 'stabilité';
    const trendPercent = q1 > 0 ? (((q4 - q1) / q1) * 100).toFixed(1) : 0;

    const mostActiveQuarter = Object.entries(statistics.byQuarter).reduce(
      (max, [q, stats]: any) =>
        stats.activities.total > max.stats.activities.total
          ? { quarter: q, stats }
          : max,
      { quarter: Quarter.Q1, stats: statistics.byQuarter[Quarter.Q1] }
    );
   ...
   */
  async generateAnnualConventionsPDF(
    conventionsByQuarter: Map<Quarter, ConventionReport[]>,
    year: number,
    statistics: any
  ): Promise<string> {
      ...
      [Quarter.Q1, Quarter.Q2, Quarter.Q3, Quarter.Q4].forEach((quarter) => {
        ...
        if (conventions.length > 0) {
          ...
          doc.fontSize(16).font('Helvetica-Bold')
            .text(quarterService.formatQuarterLabel(year, quarter), 50, 50);
          doc.moveDown(1);
          doc.fontSize(10).font('Helvetica')
   ...
   */
  async generateAnnualConventionsWORD(
    conventionsByQuarter: Map<Quarter, ConventionReport[]>,
    year: number,
    statistics: any
  ): Promise<string> {
   ...
   */
  async generateAnnualKnowledgeTransfersPDF(
    transfersByQuarter: Map<Quarter, KnowledgeTransferReport[]>,
    year: number,
    statistics: any
  ): Promise<string> {
   ...
   */
  async generateAnnualKnowledgeTransfersWORD(
    transfersByQuarter: Map<Quarter, KnowledgeTransferReport[]>,
    year: number,
    statistics: any
  ): Promise<string> {
