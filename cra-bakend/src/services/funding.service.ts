...
export class FundingService {
   ...
   */
  async createFunding(data: CreateFundingDTO, userId: string) {
    ...
    if (!isResponsible && !isParticipant) {
      throw new AuthorizationError('Vous n\'êtes pas autorisé à ajouter un financement à cette activité');
    }
    ...
    if (data.conventionId) {
      ...
      if (!convention) {
        throw new NotFoundError('Convention non trouvée');
      }
    }
   ...
   */
  async getFundings(filters: FundingFilters, userId: string, userRole: string) {
    const where: any = {};
   ...
   */
  async getFundingById(id: string, userId: string, userRole: string) {
    ...
    if (!funding) {
      throw new NotFoundError('Financement non trouvé');
    }
    ...
    if (userRole !== 'ADMINISTRATEUR') {
      ...
      if (!isResponsible && !isParticipant) {
        throw new AuthorizationError('Accès non autorisé à ce financement');
      }
    }
   ...
   */
  async updateFunding(id: string, data: UpdateFundingDTO, userId: string, userRole: string) {
    ...
    if (userRole !== 'ADMINISTRATEUR' && existing.activity.responsibleId !== userId) {
      throw new AuthorizationError('Seul le responsable de l\'activité peut modifier ce financement');
    }
   ...
   */
  async deleteFunding(id: string, userId: string, userRole: string) {
    const existing = await this.getFundingById(id, userId, userRole);

    // Seul le responsable de l'activité ou un admin peut supprimer
    if (userRole !== 'ADMINISTRATEUR' && existing.activity.responsibleId !== userId) {
      throw new AuthorizationError('Seul le responsable de l\'activité peut supprimer ce financement');
    }
   ...
   */
  async getFundingsByActivity(activityId: string, userId: string, userRole: string) {
    ...
    if (userRole !== 'ADMINISTRATEUR') {
      ...
      if (!isResponsible && !isParticipant) {
        throw new AuthorizationError('Accès non autorisé à cette activité');
      }
    }
