...
// Type pour les commentaires avec relations incluses
...
export class CommentService {
  ...
  async createComment(commentData: CreateCommentRequest, authorId: string, authorRole: string): Promise<CommentResponse> {
    ...
    switch (commentData.targetType) {
      ...
      default:
        throw new ValidationError('Type de cible non supporté');
    }
  ...
  async listComments(userId: string, userRole: string, query: CommentListQuery) {
  ...
  async getCommentById(commentId: string, userId: string, userRole: string): Promise<CommentResponse> {
  ...
  async updateComment(commentId: string, updateData: UpdateCommentRequest, userId: string, userRole: string): Promise<CommentResponse> {
    ...
    if (existingComment.authorId !== userId && userRole !== 'ADMINISTRATEUR') {
      throw new AuthError('Seul l\'auteur peut modifier son commentaire');
    }
    ...
    if (cleanContent === existingComment.content.trim()) {
      throw new ValidationError('Le nouveau contenu est identique au contenu actuel');
    }
  ...
  async deleteComment(commentId: string, userId: string, userRole: string): Promise<void> {
    ...
    if (!canDelete) {
      throw new AuthError('Permissions insuffisantes pour supprimer ce commentaire');
    }
  ...
  async getCommentStats(userId: string, userRole: string, targetType?: string, targetId?: string): Promise<CommentStatsResponse> {
    // Construire les filtres selon les permissions
    let whereClause: any = {};

    if (targetType && targetId) {
      // Vérifier l'accès à la cible spécifique
      await this.validateTargetAccess(targetType as any, targetId, userId, userRole);
    ...
    const authorIds = commentsByAuthor.map((item: any) => item.authorId);
    ...
    const enrichedByAuthor = commentsByAuthor.map((item: any) => {
  ...
  private async validateTargetAccess(targetType: 'project' | 'activity' | 'task', targetId: string, userId: string, userRole: string): Promise<void> {
    switch (targetType) {
      case 'project':
        ...
        const hasProjectAccess = userRole === 'ADMINISTRATEUR' ||
                                ...
                                project.participants.some((p: any) => p.isActive);
      ...
      case 'activity':
        ...
        const hasActivityAccess = userRole === 'ADMINISTRATEUR' ||
                                 ...
                                 activity.project.participants.some((p: any) => p.isActive);
      ...
      case 'task':
        ...
        const hasTaskAccess = userRole === 'ADMINISTRATEUR' ||
                             ...
                             (task.project && (
                               ...
                               task.project.participants.some((p: any) => p.isActive)
                             )) ||
                             (task.activity?.project && (
                               task.activity.project.creatorId === userId ||
                               task.activity.project.participants.some((p: any) => p.isActive)
                             ));
  ...
  private async buildAccessFilters(targetType: 'project' | 'activity' | 'task', userId: string, userRole: string): Promise<any[]> {
  ...
  private getTargetTypeAndId(comment: CommentWithRelations): { type: 'project' | 'activity' | 'task', id: string } {
    if (comment.projectId) {
      ...
      throw new ValidationError('Type de cible du commentaire non déterminable');
    }
  }

  // Vérifier les permissions de suppression
  private checkDeletePermissions(comment: CommentWithRelations, userId: string, userRole: string): boolean {
    ...
    if (comment.project?.creatorId === userId) return true;
    
    // Créateur du projet peut supprimer les commentaires sur les activités de son projet
    if (comment.activity?.project?.creatorId === userId) return true;
    
    // Créateur de la tâche peut supprimer les commentaires sur sa tâche
    if (comment.task?.creatorId === userId) return true;
    
    return false;
  }

  // Grouper les commentaires par jour
  private groupCommentsByDay(recentComments: any[]): Array<{ date: string; count: number }> {
    const grouped = recentComments.reduce((acc: Record<string, number>, item: any) => {
  ...
  private formatCommentResponse(comment: CommentWithRelations, targetType: string, targetId: string, userId: string, userRole: string): CommentResponse {
