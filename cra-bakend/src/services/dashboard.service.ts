...
export class DashboardService {
  ...
  async getDashboardData(userId: string, userRole: string, query: DashboardQuery): Promise<DashboardResponse> {
    ...
    const [
      ...
      documentStats,
      activityStats,
      formStats // Nouvelle statistique
    ] = await Promise.all([
  ...
  private async getProjectStatistics(userId: string, userRole: string, query: DashboardQuery) {
    ...
    statusStats.forEach((stat: { status: string; _count: number }) => {
      if (stat.status in byStatus) {
        byStatus[stat.status as keyof typeof byStatus] = stat._count;
      }
    });
  ...
  private async getTaskStatistics(userId: string, query: DashboardQuery) {
    const now = new Date();
    ...
    priorityStats.forEach((stat: { priority: string; _count: number }) => {
      if (stat.priority in byPriority) {
        byPriority[stat.priority as keyof typeof byPriority] = stat._count;
      }
    });
  ...
  private async getDocumentStatistics(userId: string, userRole: string, query: DashboardQuery) {
    ...
    typeStats.forEach((stat: { type: string; _count: number }) => {
      if (stat.type in byType) {
        byType[stat.type as keyof typeof byType] = stat._count;
      }
    });
  ...
  private async getActivityStatistics(userId: string, userRole: string, query: DashboardQuery) {
    ...
    const monthlyStats = new Map<string, { count: number; completedCount: number }>();
  ...
  private async getFormStatistics(userId: string, userRole: string, query: DashboardQuery) {
    switch (userRole) {
  ...
  private async getTechnicianFormStats(userId: string, query: DashboardQuery) {
    const [
      totalForms,
      activeForms,
      totalResponses,
    ...
    return {
      created: {
        ...
        responsesByForm: responsesByForm.map((response: any) => ({
  ...
  private async getParticipantFormStats(userId: string, query: DashboardQuery) {
    const [
      formsToComplete,
      responsesSubmitted,
      pendingForms,
    ...
    return {
      participation: {
        ...
        recentResponses: recentResponses.map((response: any) => ({
  ...
  private async getAdminFormStats(_query: DashboardQuery) {
    const [
      totalForms,
      activeForms,
      totalResponses,
    ...
    return {
      overview: {
        ...
        uniqueRespondents,
        averageResponsesPerForm,
        topForms: topForms.map((form: any) => ({
          id: form.id,
          title: form.title,
  ...
  private async getBasicFormStats(userId: string) {
  ...
  private calculateSummaryMetrics(
    ...
    const recentActivityCount = activityStats.byMonth.slice(-2).reduce((sum: number, month: any) => sum + month.count, 0);
    const previousActivityCount = activityStats.byMonth.slice(-4, -2).reduce((sum: number, month: any) => sum + month.count, 0);
    
    let trending: { direction: 'up' | 'down' | 'stable'; percentage: number; period: string };
  ...
  private async getResponsesTrend(days: number) {
    const startDate = new Date();
  ...
  async getFormPerformanceMetrics(userId: string, userRole: string, period: number = 30) {
    const startDate = new Date();
  ...
  private async getFormCompletionRates(userId: string) {
