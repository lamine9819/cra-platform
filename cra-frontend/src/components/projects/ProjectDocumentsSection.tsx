// src/components/projects/ProjectDocumentsSection.tsx
...
import { useProjectDocuments } from '../../contexts/DocumentContext';
...
export const ProjectDocumentsSection: React.FC<ProjectDocumentsSectionProps> = ({
  ...
  const handleDocumentAction = async (action: string, document: DocumentResponse) => {
    switch (action) {
      case 'download':
        try {
          // Utiliser le service pour télécharger
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            {documents.length} document{documents.length > 1 ? 's' : ''} dans {projectTitle}
          </p>
        </div>
      ...
      {showUpload && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-lg font-medium text-gray-900">Ajouter des documents au projet</h4>
            <button
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              {documents.length === 0 
                ? 'Ce projet ne contient pas encore de documents.'
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredDocuments.map((document) => (
                  <DocumentCard
                    ...
                    showActions={true}
                    showContext={false} // Pas besoin d'afficher le contexte dans la page du projet
                  />
                ))}
              </div>
            ) : (
